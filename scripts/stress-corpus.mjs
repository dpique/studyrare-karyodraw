...
export const CORPUS = [
  ...
  { k: '46,XY,del(5)(p15.3p15.2)', group: 'hole', expect: 'draw', why: 'Breakpoints given from the telomere inward; ISCN orders them centromere first. The same segment is bounded either way, so the drawing is right and only the spelling is not.', watch: 'Draws, with the rule and the corrected spelling. A judgment call: withholding a correct picture over the order of two bands would be the wrong trade, which is how listing order is handled too. dup() is deliberately excluded, because there the order distinguishes a direct duplication from an inverted one.' },
